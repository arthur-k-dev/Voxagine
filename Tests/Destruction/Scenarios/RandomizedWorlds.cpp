 ...
 * exactly. */
class RandomizedWorld : public Scenario
{
public:
	explicit RandomizedWorld(uint32_t uiIndex) :
		m_uiIndex(uiIndex),
		m_Name("randomized-world/" + std::to_string(uiIndex))
	{
	}
	...
	void Configure(DestructionConfig& config) const override
	{
		...
		config.uiTicks = 240 + TestTiming::k_uiSettleTicks;
	...
	void Build(VoxelWorldHarness& world, const DestructionConfig& config) const override
	{
		...
		for (uint16_t uiSlot = 1; uiSlot <= 12; ++uiSlot)
		{
			...
			world.FillBox(v3Min, v3Size, TestColours::k_uiStone,
			              bProtect ? DestructionRun::k_uiProtectedSlot : uiSlot);
		}
	}
...
namespace
{
	const bool s_bRegisteredRandom = []()
	{
		for (uint32_t i = 1; i <= 24; ++i)
			ScenarioRegistry::Get().Add(std::unique_ptr<Scenario>(new RandomizedWorld(i)));
