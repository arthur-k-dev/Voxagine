...
VOXAGINE_CHECK(SphericalDestruction, ClearsExactlyTheSphere)
{
	...
	for (uint32_t uiX = 0; uiX < k_v3Size.x; ++uiX)
	{
		...
		CHECK_EQ(world.Grid().GetCell(uiX, uiY, uiZ).IsActive(), !bInside)
			<< "at " << uiX << "," << uiY << "," << uiZ;
	}

	CHECK_EQ(result.uiDestroyed, uiExpected);
	CHECK_EQ(recorder.positions.size(), uiExpected);
...
/* Ledger D4: the colour handed to the debris spawner comes from the CPU voxel.
   ...
   destroyed voxel. */
VOXAGINE_CHECK(SphericalDestruction, ReportsTheColourThatWasThere)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	REQUIRE_EQ(recorder.positions.size(), 4u * 4u * 4u);
	...
	for (size_t i = 0; i < recorder.positions.size(); ++i)
	{
		...
		CHECK_EQ(recorder.colours[i], uiExpected);
	}
}

VOXAGINE_CHECK(SphericalDestruction, NeverTouchesTheGroundLayer)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	for (uint32_t uiZ = 0; uiZ < k_v3Size.z; ++uiZ)
	for (uint32_t uiX = 0; uiX < k_v3Size.x; ++uiX)
		CHECK_TRUE(world.Grid().GetCell(uiX, 0, uiZ).IsActive()) << "ground at " << uiX << "," << uiZ;
}
...
/* Ledger D2. A negative or non-finite radius used to reach `(uint32_t)fRadius *
   ...
   (D1). */
VOXAGINE_CHECK(SphericalDestruction, RejectsNonsenseRadii)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	world.FillBox(UVector3(0, 1, 0), UVector3(k_v3Size.x, k_v3Size.y - 1, k_v3Size.z), k_uiStone, 1);
	                       ...
	                       std::numeric_limits<float>::infinity() })
	{
		...
		CHECK_TRUE(result.bRejected) << "radius " << fRadius;
		CHECK_EQ(result.uiDestroyed, 0u);
	}
	...
	CHECK_TRUE(nan.bRejected);
	CHECK_EQ(world.Hash(), uiBefore);
	CHECK_TRUE(recorder.positions.empty());
}

VOXAGINE_CHECK(SphericalDestruction, ClampsAnAbsurdRadius)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	world.FillBox(UVector3(0, 1, 0), UVector3(k_v3Size.x, k_v3Size.y - 1, k_v3Size.z), k_uiStone, 1);
	...
	CHECK_TRUE(result.bRadiusClamped);
	CHECK_FALSE(result.bRejected);
	...
	CHECK_GT(result.uiDestroyed, 0u);
	CHECK_EQ(world.Validate(), 0u);
}

VOXAGINE_CHECK(SphericalDestruction, LeavesVoxelsTheOwnerProtects)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	CHECK_EQ(result.uiProtected, 4u * 4u * 4u);

	for (uint32_t uiZ = 30; uiZ < 34; ++uiZ)
	for (uint32_t uiY = 8; uiY < 12; ++uiY)
	for (uint32_t uiX = 30; uiX < 34; ++uiX)
		CHECK_TRUE(world.Grid().GetCell(uiX, uiY, uiZ).IsActive());

	CHECK_FALSE(world.Grid().GetCell(25, 5, 25).IsActive());
	CHECK_EQ(world.Validate(), 0u);
}

VOXAGINE_CHECK(SphericalDestruction, SurvivesASphereOverTheWindowEdge)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	world.FillBox(UVector3(0, 1, 0), UVector3(k_v3Size.x, k_v3Size.y - 1, k_v3Size.z), k_uiStone, 1);
	...
	CHECK_FALSE(world.Grid().GetCell(1, 1, 1).IsActive());
	CHECK_FALSE(world.Grid().GetCell(62, 30, 62).IsActive());
	CHECK_EQ(world.Validate(), 0u);
}
...
VOXAGINE_CHECK(SphericalDestruction, SeedsOnlyWhatLostANeighbour)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	world.FillGround(k_uiStone);
	...
	REQUIRE_GT(recorder.positions.size(), 0u);
	...
	{
		REQUIRE_NE(uiSeed, IntegrityChecker::k_uiInvalidHash);

		const Vector3 v3 = IntegrityChecker::HashToPosition(uiSeed);

		CHECK_TRUE(world.Grid().GetCell(
		...
		CHECK_LT(v3.x, 40.f) << "seeded a structure this burst never touched";
	}
...
VOXAGINE_CHECK(SphericalDestruction, SeedsInsideTheHoleAreFilteredOut)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	world.FillBox(UVector3(0, 1, 0), UVector3(k_v3Size.x, k_v3Size.y - 1, k_v3Size.z), k_uiStone, 1);
	...
	/* Twenty-six candidates per destroyed voxel - the checker walks
	   ...
	   survives the filter. */
	CHECK_EQ(uiCandidates, recorder.positions.size() * 26u);
	CHECK_LT(seeds.size(), uiCandidates / 4u);
...
VOXAGINE_CHECK(SphericalDestruction, TheSharedHashRejectsPositionsItCannotRepresent)
{
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(-1.f, 0.f, 0.f)), IntegrityChecker::k_uiInvalidHash);
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(0.f, -1.f, 0.f)), IntegrityChecker::k_uiInvalidHash);
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(0.f, 0.f, -0.5f)), IntegrityChecker::k_uiInvalidHash);
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(70000.f, 0.f, 0.f)), IntegrityChecker::k_uiInvalidHash);
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(std::numeric_limits<float>::quiet_NaN(), 0.f, 0.f)),
	          IntegrityChecker::k_uiInvalidHash);

	/* The two overloads have to agree, or the consolidation moved the answer. */
	CHECK_EQ(IntegrityChecker::PositionToHash(Vector3(12.f, 34.f, 56.f)),
	          IntegrityChecker::PositionToHash(12u, 34u, 56u));

	CHECK_EQ(IntegrityChecker::HashToPosition(IntegrityChecker::PositionToHash(12u, 34u, 56u)),
