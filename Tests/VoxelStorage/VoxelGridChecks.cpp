...
VOXAGINE_CHECK(VoxelGrid, GetChunkAgreesWithGetVoxel)
{
	...
	for (const Vector3& v3Origin : origins)
	{
		for (const Vector3& v3Size : sizes)
		{
			...
			REQUIRE_TRUE(world.Grid().GetChunk(voxels.data(), v3Origin, v3Size, true, slots.data()))
			...
			for (uint32_t uiX = 0; uiX < static_cast<uint32_t>(v3Size.x); ++uiX)
			{
				...
				REQUIRE_NE(voxels[uiIndex], nullptr)
					<< "origin " << v3Origin.x << "," << v3Origin.y << "," << v3Origin.z
					<< " size " << v3Size.x << "," << v3Size.y << "," << v3Size.z
					<< " at " << uiWorldX << "," << uiWorldY << "," << uiWorldZ;

				CHECK_EQ(voxels[uiIndex]->Color, cell.GetColor())
					<< "origin " << v3Origin.x << "," << v3Origin.y << "," << v3Origin.z
					<< " size " << v3Size.x << "," << v3Size.y << "," << v3Size.z
					<< " at " << uiWorldX << "," << uiWorldY << "," << uiWorldZ;

				CHECK_EQ(slots[uiIndex], cell.GetSlot())
					<< "origin " << v3Origin.x << "," << v3Origin.y << "," << v3Origin.z
					<< " at " << uiWorldX << "," << uiWorldY << "," << uiWorldZ;
			}
		}
...
VOXAGINE_CHECK(VoxelGrid, OutOfBoundsReadsReportNothing)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);

	CHECK_EQ(world.Grid().GetVoxel(k_v3Size.x, 0, 0), nullptr);
	CHECK_EQ(world.Grid().GetVoxel(0, k_v3Size.y, 0), nullptr);
	CHECK_EQ(world.Grid().GetVoxel(0, 0, k_v3Size.z), nullptr);

	CHECK_FALSE(static_cast<bool>(world.Grid().GetCell(k_v3Size.x, 0, 0)));
	CHECK_FALSE(static_cast<bool>(world.Grid().GetCell(0, k_v3Size.y, 0)));
	CHECK_FALSE(static_cast<bool>(world.Grid().GetCell(0, 0, k_v3Size.z)));
}
...
VOXAGINE_CHECK(VoxelGrid, OwnerSlotsAreStableAndUnique)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	CHECK_NE(uiFirst, VoxelOwnerVolume::k_uiNoOwnerSlot);
	CHECK_NE(uiSecond, VoxelOwnerVolume::k_uiNoOwnerSlot);
	CHECK_NE(uiFirst, uiSecond);

	CHECK_EQ(grid.AcquireOwnerSlot(1001), uiFirst);
	CHECK_EQ(grid.FindOwnerSlot(1001), uiFirst);
	CHECK_EQ(grid.ResolveOwnerSlot(uiFirst), 1001u);

	/* An entity that has never stamped anything must not consume a slot just by
	   being asked about. */
	CHECK_EQ(grid.FindOwnerSlot(9999), VoxelOwnerVolume::k_uiNoOwnerSlot);
	CHECK_EQ(grid.ResolveOwnerSlot(VoxelOwnerVolume::k_uiNoOwnerSlot), 0u);
	...
	CHECK_EQ(grid.ResolveOwnerSlot(VoxelOwnerVolume::k_uiReservedSlot), 0u);

	for (uint64_t uiEntity = 2000; uiEntity < 2064; ++uiEntity)
		CHECK_NE(grid.AcquireOwnerSlot(uiEntity), VoxelOwnerVolume::k_uiReservedSlot);
}

VOXAGINE_CHECK(VoxelGrid, WorldToGridIsTheInverseOfGridToWorld)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelGrid& grid = world.Grid();
