...
class StackedScenario : public SelftestScenario
{
public:
	const char* Name() const override { return "stacked"; }
	...
	void Build(VoxelWorldHarness& world, const SelftestConfig& config) const override
	{
		...
		for (uint32_t uiLevel = 0; uiLevel < 4; ++uiLevel)
		{
			...
			world.FillBox(UVector3(30 + uiInset, uiY, 30 + uiInset),
			              ...
			              SelftestColours::k_uiStone, uiSlot);
			++uiSlot;
		}
...
VOXAGINE_SELFTEST_SCENARIO(StackedScenario)
