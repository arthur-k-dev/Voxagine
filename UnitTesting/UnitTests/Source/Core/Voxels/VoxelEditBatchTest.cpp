...
TEST(VoxelEditBatch, SetReachesEveryRepresentation)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());

	ASSERT_TRUE(batch.Set(Vector3(5.f, 7.f, 9.f), k_uiStone, 3));
...
TEST(VoxelEditBatch, ClearAlsoClearsTheOwner)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());

	batch.Set(Vector3(5.f, 7.f, 9.f), k_uiStone, 42);
	ASSERT_EQ(world.Grid().GetCell(5, 7, 9).GetSlot(), 42);

	ASSERT_TRUE(batch.Clear(Vector3(5.f, 7.f, 9.f)));
...
TEST(VoxelEditBatch, OverwritingDoesNotMoveTheBrickCount)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());
	...
	EXPECT_EQ(world.Grid().GetCell(5, 7, 9).GetSlot(), 2);
	EXPECT_EQ(world.Validate(), 0u);
}

TEST(VoxelEditBatch, RejectsOutOfBoundsAndNegativePositions)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());
	...
	EXPECT_FALSE(batch.Set(Vector3(static_cast<float>(k_v3Size.x), 0.f, 0.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(0.f, static_cast<float>(k_v3Size.y), 0.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(0.f, 0.f, static_cast<float>(k_v3Size.z)), k_uiStone, 1));
	...
	/* Negative has to be caught *before* the cast, or -1 becomes 4294967295 and
	   ...
	   buffer. */
	EXPECT_FALSE(batch.Set(Vector3(-1.f, 0.f, 0.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(0.f, -1.f, 0.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Clear(Vector3(0.f, 0.f, -1.f)));

	EXPECT_EQ(world.Hash(), uiBefore);
	EXPECT_EQ(batch.GetWrites(), 0u);
	EXPECT_EQ(batch.GetRejected(), 6u);
}
...
TEST(VoxelEditBatch, RejectsNonFinitePositions)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());
	...
	EXPECT_FALSE(batch.Set(Vector3(fNaN, 1.f, 1.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(1.f, fNaN, 1.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(1.f, 1.f, fNaN), k_uiStone, 1));
	EXPECT_FALSE(batch.Set(Vector3(fInf, 1.f, 1.f), k_uiStone, 1));
	EXPECT_FALSE(batch.Clear(Vector3(-fInf, 1.f, 1.f)));

	EXPECT_EQ(world.Hash(), uiBefore);
	EXPECT_EQ(batch.GetNonFiniteRejections(), 5u);
}

TEST(VoxelEditBatch, RegistersOwnerlessSetsAsLooseVoxels)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	ASSERT_EQ(sink.Registered().size(), 1u);
	EXPECT_EQ(sink.Registered()[0], Vector3(4.f, 5.f, 6.f));

	/* An owned voxel has a renderer submitting a proxy for it already. */
	batch.Set(Vector3(7.f, 5.f, 6.f), k_uiStone, 9);
	EXPECT_EQ(sink.Registered().size(), 1u);
...
TEST(VoxelEditBatch, ClearRegionClampsToTheWindow)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	...
	EXPECT_EQ(uiCleared, 4u * 4u * 4u);
	EXPECT_FALSE(world.Grid().GetCell(47, 23, 47).IsActive());
	EXPECT_TRUE(world.Grid().GetCell(43, 23, 47).IsActive());
	EXPECT_EQ(world.Validate(), 0u);
}

TEST(VoxelEditBatch, ReportsTheBricksItTouched)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());

	EXPECT_TRUE(batch.GetDirtyBricks().empty());
	...
	EXPECT_LT(dirty[0], dirty[1]);
	EXPECT_EQ(dirty[0], world.Bricks().VoxelToBrick(world.VoxelID(1, 1, 1)));
	EXPECT_EQ(dirty[1], world.Bricks().VoxelToBrick(world.VoxelID(20, 1, 1)));
}
...
/* The strongest statement available about the batch: run one edit script
   ...
   golden hash would. */
TEST(VoxelEditBatch, AgreesWithTheReferenceWriter)
{
	...
	EXPECT_EQ(batched.CountOccupied(), reference.CountOccupied());
	EXPECT_EQ(batched.Validate(), 0u);
...
TEST(VoxelEditBatch, ADetachedChunkAcceptsNoWrites)
{
	VoxelWorldHarness world(k_v3Size, k_v3ChunkSize);
	VoxelEditBatch batch(world.MakeEditTarget());

	ASSERT_TRUE(batch.Set(Vector3(20.f, 1.f, 4.f), k_uiStone, 1));

	/* Chunk (1, 0) covers x in [16, 32). */
	world.Grid().SetChunkStorage(UVector2(1, 0), nullptr, nullptr);

	EXPECT_FALSE(static_cast<bool>(world.Grid().GetCell(20, 1, 4)));
	EXPECT_FALSE(world.Grid().GetCell(20, 1, 4).IsActive());
	EXPECT_EQ(world.Grid().GetVoxel(20, 1, 4), nullptr);

	EXPECT_FALSE(batch.Set(Vector3(20.f, 1.f, 4.f), k_uiWood, 2));
	EXPECT_FALSE(batch.Clear(Vector3(20.f, 1.f, 4.f)));

	/* The chunk beside it is untouched by the detach. */
	EXPECT_TRUE(batch.Set(Vector3(4.f, 1.f, 4.f), k_uiWood, 2));
}
